...
import { buildDailyRecordsPath, getDailyRecordTab } from '@/lib/daily-records/tabs'
...
export default async function DailyRecordsPickerPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Daily Records</h1>
        <p className="text-sm text-muted-foreground">
          Select a property to manage water, electricity, and daily wastage records
        </p>
      </div>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {properties.map((property) => {
          ...
          return (
            <Link key={property.id} href={buildDailyRecordsPath(property.id, activeTab)}>
