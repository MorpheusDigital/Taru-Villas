...
describe.skipIf(process.env.RUN_GOOGLE_REVIEW_DB_TESTS!=='true')('unified dashboard snapshot (read only)',()=>{
  it('loads complete authorized evidence with current synthesis and equal source weighting',async()=>{
    ...
    expect(all.filter(entry=>entry.source==='internal')).toHaveLength(2)
    ...
    const internal=summary.sources.find(s=>s.source==='internal')!
    const google=summary.sources.find(s=>s.source==='google')!
    expect(internal.count).toBe(2)
    expect(internal.weight).toBe(.5)
    expect(google.weight).toBe(.5)
    expect(summary.score).toBeCloseTo((internal.score!+google.score!)/2)
